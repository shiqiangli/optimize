...
class GenAlg {
public:
	vector <Genome> vecPop; //这个容器将储存每一个个体的染色体
	...
	GenAlg(); //构造函数
	void Reset(); //初始化变量
	void init(int popsize, double MutRate, double CrossRate, int GenLenght,double LeftPoint,double RightPoint); //初始化函数
...
class GenEngine {
public:
	GenEngine(const int& popsize,const double& mutationRate,const double& crossoverRate,const int&numGen,const int&generation,const double& leftPoint, const double& rightPoint):genAlg(),curve(),m_population() {
		g_popsize=popsize;
